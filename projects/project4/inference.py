...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    def inferenceByVariableElimination(
        ...
        for factor in eliminationOrder:
            factors, joinedFactor = joinFactorsByVariable(factors, factor)
            if len(joinedFactor.unconditionedVariables()) >= 2:
                factors.append(eliminate(joinedFactor, factor))
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for ghostPos in ghostPositions:
            self.beliefs[ghostPos] *= self.getObservationProb(
                observation, 
                pacmanPosition, 
                ghostPos, 
                jailPosition
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for ghostPos in ghostPositions:
            newPosDist[ghostPos] = self.getPositionDistribution(gameState, ghostPos)

        for ghostPos in ghostPositions:
            beliefs[ghostPos] = sum(
                newPosDist[prevPos][ghostPos] * self.beliefs[prevPos]
...
class ParticleFilter(InferenceModule):
    ...
    def getBeliefDistribution(self):
        ...
        return DiscreteDistribution(counts)
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for particle in self.particles:
            beliefs[particle] += self.getObservationProb(
                observation, 
                pacmanPosition, 
                particle, 
                jailPosition
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        legalParticles = [(a,b) for a in self.legalPositions for b in self.legalPositions]

        random.shuffle(legalParticles)
        
        self.particles = legalParticles[:len(legalParticles)]
        "*** END YOUR CODE HERE ***"
