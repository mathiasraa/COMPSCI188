...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # (position, (corner1Visited, corner2Visited, ...))
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [
            ...
            if not hitsWall:
                # Successor is corner, or has already been visited
                boolean = [
                    state[1][i] or ((nextx, nexty) == corner)
                    for i, corner in enumerate(self.corners)
                ]

                successors.append((((nextx, nexty), tuple(boolean)), action, 1))
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    # Calculate manhattan distance for all non-visited corners
    distances = [
        abs(state[0][0] - x) + abs(state[0][1] - y)
        for i, (x, y) in enumerate(corners)
        if not state[1][i]
    ]

    # Return max distance
    return max(distances or [0])
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    def cachedDistance(p1, p2, state):
        fromCache = problem.heuristicInfo.get((p1, p2))

        if fromCache:
            return fromCache
        else:
            distance = mazeDistance(p1, p2, state)
            problem.heuristicInfo[p1, p2] = distance
            return distance

    # Calculate maze distance to all dots
    distances = [
        cachedDistance(
            (x, y),
            position,
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state: Tuple[int, int]):
        ...
        "*** YOUR CODE HERE ***"
        # Every state which exists in a dot coordinate is a goal
